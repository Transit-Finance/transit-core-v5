...
contract TransitAggregateBridgeV5Permit2 is Ownable {
    ...
    mapping(address => mapping(address => bool)) private _permit2_allowed;
    mapping(address => mapping(address => bool)) private _permit2_approves;
    ...
    event ChangePermit2Allowed(address[] permit2, address[] permit2Spender);
    event ResetPermit2Approve(address indexed permit2, address indexed token, address indexed caller);
    ...
    function permit2Approves(address token, address caller) public view returns (bool) {
        return _permit2_approves[token][caller];
    }

    function permit2Allowed(address permit2, address caller) public view returns (bool) {
        return _permit2_allowed[permit2][caller];
    }
    ...
    function changePermit2Allowed(address[] calldata permit2, address[] calldata permit2Spender) public onlyExecutor {
        for (uint i; i < permit2.length; i++) {
            _permit2_allowed[permit2[i]][permit2Spender[i]] = !_permit2_allowed[permit2[i]][permit2Spender[i]];
        }
        emit ChangePermit2Allowed(permit2, permit2Spender);
    }
    ...
    function resetPermit2Approve(address permit2, address[] calldata callers, address[] calldata tokens) public onlyExecutor {
        require(callers.length == tokens.length, "TransitAggregateBridgeV5: invalid data");
        for (uint i; i < tokens.length; i++) {
            _permit2_approves[tokens[i]][callers[i]] = false;
            IPermit2(permit2).approve(tokens[i], callers[i], 0, 0);
            emit ResetPermit2Approve(permit2, tokens[i], callers[i]);
        }
    }
    ...
    function callbytes(CallbytesDescription calldata desc) external payable {
        ...
        for (uint index; index < callSize; index++) {
            ...
            if (_permit2_allowed[approveAddress][aggregateDesc.callers[index]]) {
                if (!_permit2_approves[desc.srcToken][aggregateDesc.callers[index]]) {
                    IPermit2(approveAddress).approve(desc.srcToken, aggregateDesc.callers[index], type(uint160).max, type(uint48).max);
                    _permit2_approves[desc.srcToken][aggregateDesc.callers[index]] = true;
                }
            }
        ...
        if (!TransferHelper.isETH(desc.srcToken)) {
            if (IERC20(desc.srcToken).balanceOf(address(this)) > 0) {
                IERC20(desc.srcToken).transfer(transitRouter(), IERC20(desc.srcToken).balanceOf(address(this)));
            }
        }
    }
